...
48 وثيقة، 3 تنسيقات، 15 لغة — Apple M-series، بناء `--release`. الأرقام أدناه مُقاسة باستخدام **محلّل الرموز الحقيقي `cl100k` BPE** (وليس الاستدلال المرجعي الذاتي — انظر التحليل). المنهجية الكاملة وتفصيل صدقية الرموز: [`docs/EVALUATION.md`](../EVALUATION.md)
...
> ⚠️ الرقم الإجمالي تهيمن عليه عملية إزالة ترميز HTML. **27.4% لـ Markdown هو معدل الضغط الحقيقي.** PlainText سلبي صافٍ في وضع Semantic بسبب التكلفة الإضافية الهيكلية. انظر [`docs/EVALUATION.md`](../EVALUATION.md) للحقيقة حسب التنسيق.
...
قياس في بناء إصدار (`cargo build --release`)، Apple M-series، 48 وثيقة عبر Markdown/HTML/PlainText. جميع أرقام التخفيض مُقاسة باستخدام **محلّل الرموز الحقيقي `cl100k` BPE** (وليس الاستدلال المرجعي الذاتي). انظر [`docs/EVALUATION.md`](../EVALUATION.md) للمنهجية الكاملة والتفصيل حسب التنسيق.

| المقياس | المقيَّس | ملاحظات |
|---------|---------|---------|
| الإنتاجية (ذروة Markdown فقط) | **10,975 tok/ms** | ≈75 مرة أسرع من الخط الأساسي لتحليل Python؛ ذروة تنسيق واحد |
| الإنتاجية (مجموع مجموعة البيانات) | **~1,070 tok/ms** | موزون عبر جميع الوثائق الـ 48 / 3 تنسيقات (BPE) — انظر جدول قياسات الأداء |
| تخفيض Semantic | **27.4%** (Markdown) | معدل الضغط الحقيقي؛ ضمن النطاق المستهدف 15–30% |
| تخفيض Compressed | **69.4%** (Markdown) | تكيّفي مع الميزانية، ≥ PruneLowImportance مضمون |
| تغطية كلمات Lossless | **99.0% متوسط** | عبر جميع التنسيقات واللغات |
| تخفيض HTML | **98.7%** | يعكس إزالة التكلفة الإضافية للترميز (nav/السكريبتات/الأنماط) |
| الدعم متعدد اللغات | 15 لغة مُختبَرة | AR/DE/ES/FR/HI/IT/JA/KO/NL/PL/PT/RU/SV/TR/ZH — 99.0% تغطية كلمات متوسطة |

شغّل مجموعة التقييم بنفسك:

```bash
make eval          # JSON منظم (BPE + استدلالي؛ يُستهلك بواسطة `epic eval`)
make eval-report   # جدول لكل ملف قابل للقراءة بشرياً + ملخص
```
