...
48 документов, 3 формата, 15 языков — Apple M-series, сборка `--release`. Приведённые ниже цифры измерены с **реальным `cl100k` BPE-токенайзером** (не самоописательной эвристикой — см. анализ). Полная методология и разбор честности токенов: [`docs/EVALUATION.md`](../EVALUATION.md)
...
> ⚠️ Общий показатель определяется удалением HTML-разметки. **27.4% для Markdown — это подлинный уровень сжатия.** Для PlainText в режиме Semantic наблюдается чистое увеличение из-за структурных накладных расходов. Подробности по каждому формату: [`docs/EVALUATION.md`](../EVALUATION.md).
...
Бинарник и хук PreToolUse автоматически устанавливаются при следующем запуске сессии — дополнительная настройка не требуется.
...
Хук PreToolUse регистрируется автоматически — дальнейшие действия не требуются.
...
Измерено на сборке release (`cargo build --release`), Apple M-series, 48 документов форматов Markdown / HTML / PlainText. Все показатели сокращения измерены с **реальным `cl100k` BPE-токенайзером** (не самоописательной эвристикой). Полная методология и разбор по форматам: [`docs/EVALUATION.md`](../EVALUATION.md).

| Метрика | Измерено | Примечания |
|---------|----------|------------|
| Пропускная способность (пик по Markdown) | **10 975 tok/ms** | ≈75× быстрее базовой реализации на Python; пик для одного формата |
| Пропускная способность (агрегат по датасету) | **~1 070 tok/ms** | Взвешенное среднее по всем 48 документам / 3 форматам (BPE) — см. таблицу бенчмарков |
| Сокращение Semantic | **27.4%** (Markdown) | Подлинный уровень сжатия; в целевом диапазоне 15–30% |
| Сокращение Compressed | **69.4%** (Markdown) | Адаптивно к бюджету, ≥ PruneLowImportance гарантировано |
| Покрытие слов Lossless | **99.0% среднее** | Все форматы и языки |
| Сокращение HTML | **98.7%** | Удаление накладных расходов разметки nav/скриптов/стилей |
| Многоязычная поддержка | 15 языков протестировано | AR/DE/ES/FR/HI/IT/JA/KO/NL/PL/PT/RU/SV/TR/ZH — 99.0% покрытие слов в среднем |

Запустите набор тестов самостоятельно:

```bash
make eval          # структурированный JSON (BPE + эвристика; используется `epic eval`)
make eval-report   # читаемая таблица по файлам + сводка
```
