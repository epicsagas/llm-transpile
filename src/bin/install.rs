...
// ── Shared context-generation script ──────────────────────────────────────────

/// Generates ~/.local/share/llm-transpile/ctx-gen.sh — shared by all tools.
/// Scans the cwd for large docs and writes compressed output to stdout as JSON
/// `{"additionalContext": "..."}` (Claude/Codex/Cursor format).
const CTX_GEN_SCRIPT: &str = r####"#!/usr/bin/env bash
# Auto-generated by `transpile install`. Re-run to update.
# Scans cwd for large documentation files and outputs token-compressed context.
# Outputs {"additionalContext": "..."} for tools that support JSON hook output,
# or plain text for tools that inject hook stdout directly.
set -euo pipefail

THRESHOLD=${TRANSPILE_THRESHOLD:-8192}
JSON_OUTPUT=${TRANSPILE_JSON_OUTPUT:-1}
DOCS=(README.md ARCHITECTURE.md CLAUDE.md AGENTS.md GEMINI.md SPEC.md CONTRIBUTING.md)

BODY=""
for f in "${DOCS[@]}"; do
  [ -f "$f" ] || continue
  bytes=$(wc -c < "$f" 2>/dev/null || echo 0)
  [ "$bytes" -lt "$THRESHOLD" ] && continue
  compressed=$(transpile --input "$f" --fidelity semantic --quiet 2>/dev/null) || continue
  [ -z "$compressed" ] && continue
  BODY="${BODY}### ${f}"$'\n'"${compressed}"$'\n\n'
done
...
if [ "$JSON_OUTPUT" = "1" ]; then
  python3 -c "import json,sys; print(json.dumps({'additionalContext': sys.argv[1]}))" "$BODY"
else
  printf '%s' "$BODY"
fi
"####;

fn ctx_gen_script_path() -> PathBuf {
    let share = std::env::var("XDG_DATA_HOME")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from(home()).join(".local").join("share"));
    share.join("llm-transpile").join("ctx-gen.sh")
}

fn write_ctx_gen_script() -> PathBuf {
    let path = ctx_gen_script_path();
    std::fs::create_dir_all(path.parent().unwrap()).ok();
    std::fs::write(&path, CTX_GEN_SCRIPT).ok();
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
...
fn setup_gemini() {
    let ctx_script = write_ctx_gen_script();
    ...
    // Enable hooks
    cfg["hooksConfig"]["enabled"] = serde_json::json!(true);

    // BeforeAgent: inject compressed context at session start
    let hook_cmd = format!("bash \"{}\"", ctx_script.display());
    let hook = serde_json::json!([{
        "hooks": [{
            "name": "llm-transpile-ctx",
            "type": "command",
            "command": hook_cmd,
            "description": "Auto-compress large workspace docs and inject into context"
        }]
    }]);

    // Upsert: remove existing llm-transpile hook then re-add
    if let Some(before) = cfg["hooks"]["BeforeAgent"].as_array_mut() {
        before.retain(|h| {
            !h["hooks"].as_array()
                .map(|a| a.iter().any(|e| e["name"].as_str() == Some("llm-transpile-ctx")))
                .unwrap_or(false)
        });
        before.push(hook[0].clone());
    } else {
        cfg["hooks"]["BeforeAgent"] = hook;
    }

    std::fs::write(&settings_path, serde_json::to_string_pretty(&cfg).unwrap()).ok();
    eprintln!("  Gemini CLI: BeforeAgent hook registered in {}", settings_path.display());
}
...
fn remove_gemini() {
    ...
    std::fs::write(&path, serde_json::to_string_pretty(&cfg).unwrap()).ok();
    eprintln!("  Gemini CLI: BeforeAgent hook removed");
}
...
fn setup_codex() {
    ...
    // Ensure config.toml has codex_hooks enabled
    let config_path = codex_dir.join("config.toml");
    let config_raw = std::fs::read_to_string(&config_path).unwrap_or_default();
    if !config_raw.contains("codex_hooks") {
        let addition = "\n[features]\ncodex_hooks = true\n";
        std::fs::write(&config_path, format!("{config_raw}{addition}")).ok();
        eprintln!("  Codex CLI: enabled codex_hooks in config.toml");
    }

    let hooks_path = codex_hooks_path();
    let raw = std::fs::read_to_string(&hooks_path).unwrap_or_else(|_| r#"{"hooks":{}}"#.into());
    let mut cfg: serde_json::Value = serde_json::from_str(&raw)
        .unwrap_or(serde_json::json!({"hooks": {}}));

    // SessionStart: inject compressed workspace context
    let hook_cmd = format!("bash \"{}\"", ctx_script.display());
    ...
    if let Some(arr) = cfg["hooks"]["SessionStart"].as_array_mut() {
        arr.retain(|h| h["description"].as_str() != Some("Auto-compress large workspace docs and inject into context"));
        arr.push(new_hook);
    } else {
        cfg["hooks"]["SessionStart"] = serde_json::json!([new_hook]);
    }

    std::fs::write(&hooks_path, serde_json::to_string_pretty(&cfg).unwrap()).ok();
    eprintln!("  Codex CLI: SessionStart hook registered in {}", hooks_path.display());
}
...
fn remove_codex() {
    ...
    std::fs::write(&path, serde_json::to_string_pretty(&cfg).unwrap()).ok();
    eprintln!("  Codex CLI: SessionStart hook removed");
}

// ── OpenCode ───────────────────────────────────────────────────────────────────

/// Minimal OpenCode plugin: runs ctx-gen on BeforeAgent and injects as context.
const OPENCODE_PLUGIN: &str = r#"/**
 * llm-transpile OpenCode plugin — auto-generated by `transpile install`.
 * Compresses large workspace docs and injects them before each agent session.
 */
import { spawnSync } from "node:child_process";
import { existsSync } from "node:fs";

const CTX_SCRIPT = process.env.TRANSPILE_CTX_SCRIPT
  || `${process.env.HOME}/.local/share/llm-transpile/ctx-gen.sh`;
...
export default {
  ...
  hooks: {
    BeforeAgent(input) {
      if (!existsSync(CTX_SCRIPT)) return {};
      const result = spawnSync("bash", [CTX_SCRIPT], {
        encoding: "utf8",
        timeout: 15_000,
        env: { ...process.env, TRANSPILE_JSON_OUTPUT: "0" },
      });
...
fn opencode_plugin_path() -> PathBuf {
    ...
    cfg_dir.join("opencode").join("plugins").join("llm-transpile.js")
}

fn setup_opencode() {
    write_ctx_gen_script();
    let plugin_path = opencode_plugin_path();
    std::fs::create_dir_all(plugin_path.parent().unwrap()).ok();
    std::fs::write(&plugin_path, OPENCODE_PLUGIN).ok();
    eprintln!("  OpenCode: plugin written to {}", plugin_path.display());
}

fn remove_opencode() {
    let p = opencode_plugin_path();
    if p.exists() { std::fs::remove_file(&p).ok(); }
    eprintln!("  OpenCode: plugin removed");
}

// ── Cursor ─────────────────────────────────────────────────────────────────────

/// .cursor/hooks.json — postToolUse on all tools regenerates context.md async.
fn cursor_hooks_json(ctx_script: &std::path::Path) -> serde_json::Value {
    serde_json::json!({
        "version": 1,
        "hooks": {
            "postToolUse": [{
                "matcher": "Read",
                "command": format!(
                    "bash -c 'f=$(echo \"$HOOK_INPUT\" | python3 -c \"import json,sys; print(json.load(sys.stdin).get(\\\"tool_input\\\",{{}}).get(\\\"file_path\\\",\\\"\\\"))\" 2>/dev/null); bytes=$(wc -c < \"$f\" 2>/dev/null || echo 0); [ \"$bytes\" -gt \"${{TRANSPILE_THRESHOLD:-8192}}\" ] && bash \"{script}\" || true'",
                    script = ctx_script.display()
                ),
                "async": true,
                "description": "Auto-regenerate context.md when a large file is read"
            }]
        }
    })
}
...
for f in README.md ARCHITECTURE.md CLAUDE.md AGENTS.md SPEC.md; do
  [[ -f "$f" ]] || continue
  printf '### %s\n' "$f" >> "$OUT"
...
fn setup_cursor() {
    ...
    // Context regeneration script (manual use / initial run)
    ...
    // hooks.json — auto-regenerate on large file reads
    let hooks_path = std::path::Path::new(".cursor/hooks.json");
    let existing = std::fs::read_to_string(hooks_path).unwrap_or_default();
    let mut hooks_cfg: serde_json::Value = serde_json::from_str(&existing)
        .unwrap_or(serde_json::json!({"version": 1, "hooks": {}}));

    let new_hook = cursor_hooks_json(&ctx_script);
    if let Some(arr) = hooks_cfg["hooks"]["postToolUse"].as_array_mut() {
        arr.retain(|h| h["description"].as_str() != Some("Auto-regenerate context.md when a large file is read"));
        arr.push(new_hook["hooks"]["postToolUse"][0].clone());
    } else {
        hooks_cfg["hooks"]["postToolUse"] = new_hook["hooks"]["postToolUse"].clone();
    }

    std::fs::write(hooks_path, serde_json::to_string_pretty(&hooks_cfg).unwrap()).ok();

    // Run initial context generation
    let _ = std::process::Command::new("bash")
        .arg(".cursor/transpile-ctx.sh")
        .status();

    eprintln!("  Cursor: hooks.json written, context.md generated");
    eprintln!("  Cursor: add @.cursor/context.md to .cursorrules to auto-load");
}

fn remove_cursor() {
    for p in &[".cursor/transpile-ctx.sh", ".cursor/context.md"] {
        let path = std::path::Path::new(p);
        if path.exists() { std::fs::remove_file(path).ok(); }
    }
    // Remove our postToolUse hook from hooks.json
    let hooks_path = std::path::Path::new(".cursor/hooks.json");
    ...
    eprintln!("  Cursor: hooks and context files removed");
}
